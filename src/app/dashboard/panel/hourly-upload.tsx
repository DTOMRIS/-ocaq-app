...
export default function HourlyUpload() {
  ...
  function reset() {
    setFile(null); setRep(null); setErr(null); setResult(null); setPhase('')
  ...
  async function read() {
    if (!file) return
    setBusy(true); setErr(null); setRep(null); setResult(null)
    try {
  ...
  async function save() {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        throw new Error(`${j?.error ?? 'Yazma xətası'}${j?.detail ? ` — ${j.detail}` : ''}`)
      }
          ...
          {rep && (
            <>
              ...
              <div style={{ display: 'flex', gap: 10, alignItems: 'center' }}>
                ...
                <button onClick={save} disabled={busy || !coverEnd} style={{ marginLeft: 'auto', padding: '9px 20px', borderRadius: 10, border: 'none', background: busy || !coverEnd ? '#9a9488' : '#C8102E', color: '#fff', fontWeight: 700, cursor: busy ? 'wait' : 'pointer' }}>
