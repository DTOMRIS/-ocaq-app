 ...
 *
 * Tətbiq:
 *   TENANT_SLUG="ocaq" CONFIRM_TENANT="ocaq" CONFIRM_COUNT="37" \
...
// Personel satırları yalnız tarixi referans kimi saxlanır. OCAQ giriş hesabı
// yalnız rəhbər rolları üçün yaradılır; personel kimliyi TQTA-da yaşayır.
const LOGIN_ROSTER = managementRoster(ROSTER)

type PlannedRow = (typeof LOGIN_ROSTER)[number] & {
...
async function main() {
  ...
  const structureIssues = validateRosterStructure(LOGIN_ROSTER)
  ...
  const [allBranches, allRegions, existingUsers] = await Promise.all([
    ...
    db.select({
      ...
      email: users.email,
      role: users.role,
    }).from(users).where(inArray(users.email, LOGIN_ROSTER.map((row) => row.email))),
  ])
  ...
  const counts = roleCounts(LOGIN_ROSTER)
  console.log(`\nTenant: ${tenant.name} (${tenant.slug})`)
  console.log(`Rəhbər hesab rosteri: ${LOGIN_ROSTER.length}; yeni: ${plan.length}; mövcud və uyğun: ${skipped}`)
  console.log(`Girişsiz personel referansı: ${ROSTER.length - LOGIN_ROSTER.length} (TQTA sahibidir)`)
  ...
  if (CONFIRM_COUNT !== LOGIN_ROSTER.length) confirmations.push(`CONFIRM_COUNT ${LOGIN_ROSTER.length} olmalıdır.`)
  ...
  const expected = {
    ...
    profiles: plan.filter((row) => row.role === 'branch_manager').length,
  }
