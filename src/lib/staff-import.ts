...
export type ManagementImportRole = Exclude<ImportRole, 'staff'>
...
export function managementRoster(rows: ImportRosterRow[]) {
  return rows.filter(
    (row): row is ImportRosterRow & { role: ManagementImportRole } => row.role !== 'staff',
  )
}
