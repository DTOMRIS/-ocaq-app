...
- ✅ 37 rəhbər hesab üçün **import script** yazıldı: `src/db/import-staff.ts`
  (3 super admin + 5 bölgə müdürü + 29 filial müdürü). 26 personel sətri
  referans kimi qorunur, OCAQ giriş hesabına çevrilmir; personel kimliyi TQTA-dadır.
...
### ADIM 1 — 37 rəhbər hesabı Preview'a aç (import script)
...
2. Preflight sorgularını çalıştır ve sonucu sakla; ardından migration'ları Production'a uygula:
   `0004`, `0005`, `0006`, `0007` ve əlavəedici `0008`.
3. `main`'i tam sürüme getir (fast-forward) → otomatik deploy.
4. 37 rəhbər hesabı Production'a da aç (aynı dry-run + açık onaylarla, Production URL ile).
...
- ✅ "Şifremi unuttum" → DK SMTP ilə işləyir; göndəriş alınmasa UI xəta göstərir.
